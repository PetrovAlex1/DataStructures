...
class WordsMultiset {
private:
	HashTable<std::string, int> wordsList;
public:
	...
	void removeWord(const std::string& word, size_t times = 1);

	std::vector<std::string> getListOfElements();

	std::vector<std::string> getListOfUniqueWords() const;
};
